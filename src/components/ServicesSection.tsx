...
function defaultIllustration(icon: string | null) {
  return () => (
...
const ServicesSection = () => {
  const { tVi, t, language } = useI18n();
  const cmsServices = useCmsServices(language);

  const services = cmsServices.data?.services ?? [];
  const liveServices = services.filter((s) => s.status === "live").sort((a, b) => a.position - b.position);

  if (cmsServices.isLoading || liveServices.length === 0) {
    return (
  ...
  return (
    <section id="services" className="py-28 bg-card relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className={`grid gap-6 ${liveServices.length === 4 ? "md:grid-cols-2 lg:grid-cols-4" : "md:grid-cols-3"}`}>
          {liveServices.map((s, i) => {
            const Illustration = ILLUSTRATIONS[s.id] ?? defaultIllustration(s.icon);
            const direction: "left" | "up" | "right" = i === 0 ? "left" : i === liveServices.length - 1 ? "right" : "up";
            return (
              <ScrollReveal key={s.id} delay={i * 120} direction={direction}>
                <div className="group relative rounded-3xl border border-border/60 bg-background overflow-hidden tilt-card h-full flex flex-col">
                  ...
                  <div className="p-7 flex-1 flex flex-col">
                    {s.hero_eyebrow && (
                      <p className="text-xs font-semibold text-accent uppercase tracking-[0.15em] mb-2">{s.hero_eyebrow}</p>
                    )}
                    <h3 className="text-2xl font-bold text-navy mb-3 tracking-tight">{s.name}</h3>
                    {s.body_md && (
                      <p className="text-sm text-muted-foreground leading-relaxed mb-5 line-clamp-3">{s.body_md}</p>
                    )}
                    ...
                    {s.cta_url && (
                      ...
                      >
                        {s.cta_text || t("services.learn_more")}
