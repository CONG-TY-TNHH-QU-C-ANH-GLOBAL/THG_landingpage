...
const serviceItems: NavMenuItem[] = [
  ...
  {
    icon: Warehouse,
    titleKey: "nav.thg_warehouse",
    descKey: "nav.warehouse_desc",
    href: "/thg-warehouse",
  },
...
const pricingItems: NavMenuItem[] = [
  {
    icon: Globe,
    titleKey: "nav.intl_pricing",
    descKey: "nav.intl_pricing_desc",
    href: "/international-pricing",
  },
  {
    icon: MapPin,
    titleKey: "nav.domestic_pricing",
    descKey: "nav.domestic_pricing_desc",
    href: "/domestic-pricing",
  },
  {
    icon: Ship,
    titleKey: "nav.chinhngach_pricing",
    descKey: "nav.chinhngach_pricing_desc",
    href: "/chinh-ngach-pricing",
  },
  { icon: Tag, titleKey: "nav.catalog", descKey: "nav.catalog_desc", href: "/catalog" },
];

const eventItems: NavMenuItem[] = [
  { icon: CalendarDays, titleKey: "nav.event", descKey: "nav.event_desc", href: "/events" },
];
...
function DesktopDropdownItem({
  item,
  onClick,
}: Readonly<{ item: NavMenuItem; onClick: () => void }>) {
  ...
  return (
    ...
    >
      ...
      <div>
        <p
          className={`text-sm font-semibold text-foreground ${item.titleKey.includes("thg_") ? "notranslate" : ""}`}
        >
          {t(item.titleKey)}
        </p>
    ...
    >
      <div className="container mx-auto flex items-center justify-between h-16 lg:h-20 px-4">
        <Link to={`/${language}`} className="flex items-center gap-3 group">
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center group-hover:scale-105 group-hover:shadow-lg transition-all duration-300 overflow-hidden p-1.5 ${tone.logoBox}`}
          >
            <img
              src={thgLogo}
              alt="THG"
          ...
          <div>
            <span
              className={`text-base font-bold leading-tight tracking-tight block ${tone.brandText}`}
            >
              THG Fulfill
            </span>
            <span className={`text-[9px] tracking-[0.15em] uppercase block ${tone.brandSub}`}>
              Transport Happiness Group
            </span>
          </div>
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-1">
          ...
          >
            <button
              className={`flex items-center gap-1 px-4 py-2 text-sm font-medium transition-colors rounded-lg ${tone.navItem}`}
            >
              <span translate="no">{t("nav.services")}</span>
              <ChevronDown
                className={`w-3.5 h-3.5 transition-transform duration-300 ${openMenu === "services" ? "rotate-180" : ""}`}
              />
            </button>
          ...
          <div
            className="relative"
            onMouseEnter={() => menuEnter("pricing")}
            onMouseLeave={menuLeave}
          >
            <button
              className={`flex items-center gap-1 px-4 py-2 text-sm font-medium transition-colors rounded-lg ${tone.navItem}`}
            >
              ...
              <ChevronDown
                className={`w-3.5 h-3.5 transition-transform duration-300 ${openMenu === "pricing" ? "rotate-180" : ""}`}
              />
            </button>
          ...
          <div
            className="relative"
            onMouseEnter={() => menuEnter("community")}
            onMouseLeave={menuLeave}
          >
            <button
              className={`flex items-center gap-1 px-4 py-2 text-sm font-medium transition-colors rounded-lg ${tone.navItem}`}
            >
              <span translate="no">{t("nav.event")}</span>
              <ChevronDown
                className={`w-3.5 h-3.5 transition-transform duration-300 ${openMenu === "community" ? "rotate-180" : ""}`}
              />
            </button>
          ...
          {navItems.map((item) => (
            <Link
              ...
              className={`px-4 py-2 text-sm font-medium transition-all duration-300 rounded-lg ${
                location.pathname === lp(item.href) ? "text-primary" : tone.navItem
              }`}
            >
        ...
        <div className="flex lg:hidden items-center gap-2">
          ...
          >
            {isOpen ? (
              <X size={22} className={tone.toggleIcon} />
            ) : (
              <Menu size={22} className={tone.toggleIcon} />
            )}
          </button>
        </div>
      </div>
          ...
          <button
            onClick={() => {
              ...
              if (location.pathname === `/${language}`) {
                setTimeout(
                  () => document.getElementById("faq")?.scrollIntoView({ behavior: "smooth" }),
                  DELAYS.NAVBAR_MOBILE_SCROLL_DELAY_MS,
                );
              } else {
                globalThis.location.href = `/${language}/#faq`;
              }
