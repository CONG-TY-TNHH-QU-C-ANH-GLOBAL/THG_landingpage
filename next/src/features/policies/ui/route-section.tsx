  ...
  return (
    <section id={route.slug} className="mb-12 scroll-mt-28">
      ...
      {isRouteContentEmpty(route) ? (
        ...
        <>
          {splitSections(route.bodyMarkdown).map((section, i) => (
          ...
          {route.tables.map((table, i) => (
            <RateTable key={i} table={table} label={`${copy.tableLabel} ${i + 1}`} />
          ))}
