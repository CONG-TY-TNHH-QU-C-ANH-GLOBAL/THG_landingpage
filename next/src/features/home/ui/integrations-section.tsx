// Parity source: src/components/IntegrationsSection.tsx
...
const VISUAL_BY_NAME: Record<string, { icon: string | null; svg?: ReactNode }> = {
  Etsy: { icon: "🛍️" },
  Amazon: { icon: "📦" },
...
interface IntegrationsSectionProps {
  copy: MarketingCopy;
  /** Arrives position-sorted from the loader (FND-005) — no local sort needed. */
  integrations: readonly Integration[];
}

const IntegrationsSection = ({ copy, integrations }: IntegrationsSectionProps) => {
  const t = tFrom(copy);
      ...
      <div className="container mx-auto px-4 relative z-10">
        <ScrollReveal>
          <SectionHeader
            size="lg"
            className="mb-6"
            eyebrow={t("integrations.subtitle")}
            title={t("integrations.title")}
            titleHighlight={t("integrations.title_highlight")}
          />
        </ScrollReveal>
            ...
            {integrations.map((p, i) => {
              ...
              return (
                <ScrollReveal key={p.id} delay={i * 80}>
                  <div className={`rounded-2xl border ${p.colorClass ?? "bg-card border-border"} p-6 flex flex-col items-center gap-3 tilt-card transition-all duration-300 group`}>
