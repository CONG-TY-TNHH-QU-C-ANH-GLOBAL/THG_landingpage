// Parity source: src/components/AdvantagesSection.tsx
...
const AdvantagesSection = ({ copy }: Readonly<{ copy: MarketingCopy }>) => {
      ...
      <div className="container mx-auto px-4 relative z-10">
        <ScrollReveal>
          <SectionHeader
            size="xl"
            className="mb-20"
            eyebrow={tVi("adv.subtitle")}
            title={tVi("adv.title")}
            titleHighlight={tVi("adv.title_highlight")}
          />
        </ScrollReveal>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {advantages.map((a, i) => (
            <ScrollReveal key={a.titleKey} delay={i * 100}>
              <div
                className="flex gap-5 group p-5 rounded-2xl hover:bg-card/60 hover:shadow-lg transition-all duration-500"
